...
@section('header', 'Thêm Phòng Ban Mới')
        ...
        <form method="POST" action="{{ route('departments.store') }}" class="space-y-5">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">
                    Biên độ trễ <span class="text-gray-400 font-normal">(phút)</span> <span class="text-red-500">*</span>
                </label>
                <input type="number" name="late_tolerance" value="{{ old('late_tolerance', 15) }}" min="0" max="60" required
                       class="w-32 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none">
                <span class="text-xs text-gray-400 ml-2">Check-in trễ hơn N phút → trạng thái "Trễ"</span>
                @error('late_tolerance')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
